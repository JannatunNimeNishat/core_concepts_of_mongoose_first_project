...
const findLastStudentId = async () =>{
  ...
  export const generateStudentId = async (payload:TAcademicSemester)=>{
    ...
    const lastAdmittedStudentSemesterCode = lastAdmittedStudentId?.substring(4,6);//01 
    ...
    if(lastAdmittedStudentId && lastAdmittedStudentSemesterCode === currentAdmittedStudentSemesterCode && lastAdmittedStudentYear === currentAdmittedStudentSemesterYear){
        currentId = lastAdmittedStudentId.substring(6); // 0001
    }

    let currentAdmittedStudentId = (Number(currentId)+1).toString().padStart(4,'0');
